...
> Design notes for **compliant-checkout-rollout** — a runnable go example that Stage an e-commerce checkout feature through reviewed percentage rollouts from a small Go command.
...
- **Thin client** — a ~30-line helper that owns the base URL, the auth header, and envelope unwrapping, so call sites stay readable (e.g. `infrai.flags.set(...)`).
- **Feature code** — the actual task: checkout-feature-rollout.
...
- `flags.rollout` — mapped to `POST /v1/flags/rollout/{key}`.
